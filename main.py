...
def log(action, value, verbose):
    if verbose:
        print(f'{action}: {value}, Thread: {threading.current_thread().name}, Time: {time.time()}')
...
class RemoteSource:
    def __init__(self, rate, take, latency, latency_variance, verbose):
        ...
        self.__simulation = interval(rate, scheduler=source_scheduler).pipe(ops.map(lambda it: self.__mj_step(it)),
                                                                            ops.do_action(
                                                                                lambda it: log('Emitted', it,
                                                                                               self.__verbose)),
                                                                            ops.take(take), ops.publish())
        self.__remote_simulation = self.__simulation.pipe(ops.flat_map(
            lambda it: of(it).pipe(ops.delay(max(0, random.normalvariate(mu=latency, sigma=latency_variance))))))
    ...
    def __mj_step(self, index):
        ...
        return {'id': index, 'qpos': qpos, 'qvel': qvel, 'qfrc_bias': self.__model_data.qfrc_bias.copy(),
                'time': time.time()}
...
class PhysicsEngine:
    def __init__(self, source, strategy, rate, verbose):
        ...
        self.__physics_engine = source.pipe(ops.do_action(lambda it: log('Received', it, self.__verbose)), strategy(),
                                            ops.observe_on(mujoco_scheduler), ops.map(self.__calculate),
    ...
    def __calculate(self, value):
        ...
        return {'id': value['id'], 'qfrc_bias': list(value['qfrc_bias']),
                'qfrc_inverse': list(model_data.qfrc_inverse.copy()), 'start_time': value['time'],
                'end_time': time.time()}
    ...
    def subscribe(self):
        return self.__physics_engine.subscribe(on_next=lambda it: log('Calculated', it, self.__verbose),
...
def run(rate=0.02, take=50, network_latency=0.005, network_latency_variance=0.005, verbose=False):
    remote_source = RemoteSource(rate, take, network_latency, network_latency_variance, verbose)
