    ...
    def concat_all_inner(source: Observable[Observable[_T]]) -> Observable[_T]:
        def subscribe(observer: abc.ObserverBase[_T], scheduler: Optional[abc.SchedulerBase] = None):
            ...
            def subscribe(inner_source: Observable[_T]):
                ...
                inner_subscription.disposable = inner_source.subscribe(on_next, on_error, on_completed,
                                                                       scheduler=scheduler)
            ...
            initial_assignment.disposable = source.subscribe(on_next, observer.on_error, on_completed,
                                                             scheduler=scheduler)
            return group
...
def _concat_map_internal(source: Observable[_T1], mapper: Optional[Mapper[_T1, Any]] = None) -> Observable[Any]:
...
def concat_map(mapper: Optional[Mapper[_T1, Observable[_T2]]] = None) -> Callable[[Observable[_T1]], Observable[_T2]]:
